...
- `aurora-routing` e `aurora-workflow` são skills nativas em `.opencode/skills/`.
- Os dois arquivos também são carregados por `instructions` no `opencode.json`.
- A permissão `task` do agente primário usa allowlist e expõe somente subagentes `aurora-*` aprovados.
- Cada subagente tem descrição com gatilho de uso e `task: deny` para impedir delegação recursiva.
- A FASE -1 ativa personas; a FASE 5 chama três reviewers separados.
- `/start` confirma a ativação; `/workflow` executa as 8 fases.
